...
def signUp(request):
    ...
    c.execute(
        ...
        (username, 0, 0, 0, sync_date, 0, 0, 0, authToken, version, email, hash),
    )
    conn.commit()
    print("New sign-up")
...
def updateAccount(request):
    ...
    if auth == 200:
        ...
        authToken = secrets.token_hex(nbytes=64)
        c.execute(
            "UPDATE Leaderboard SET Token = (?), email = (?), hash = (?) WHERE Username = (?) ",
            (authToken, email, hash, username),
        )
        conn.commit()
        ...
        return HttpResponse(
            json.dumps("<h3>404 error</h3>Couldn't find user, or token invalid.")
        )
...
def changeUsername(request):
    ...
    if not taken:
        ...
        try:
            ...
            return HttpResponse(
                json.dumps("<h3>404 error</h3>Couldn't find user, or wrong password")
            )
        ...
        c.execute(
            ...
            (authToken, hash, new_username, username),
        )
        c.execute(
...
def resetPassword(request):
    ...
    try:
        ...
        # check if it exists
        c.execute(
            "SELECT Username FROM Leaderboard WHERE Username = (?) AND lower(email) = (?)",
            (username, email),
        ).fetchone()[0]

        c.execute(
            "UPDATE Leaderboard SET emailReset = (?) WHERE Username = (?) AND lower(email) = (?)",
...
def auth_user(user, token):
    ...
    c = conn.cursor()
    t = c.execute(
        "SELECT Token FROM Leaderboard WHERE Username = (?)", (user,)
    ).fetchone()
    if not t:
        return 404
...
def sync(request):
    ...
    try:
        sus = c.execute(
            "SELECT suspended FROM Leaderboard WHERE Username = (?)", (User,)
        ).fetchone()[0]
        if sus:
            return HttpResponse(
                f"<h3>Account suspended</h3>This account was suspended due to the following reason:<br><br>{sus}<br><br>Please write an e-mail to leaderboard_support@protonmail.com or a message me on <a href='https://www.reddit.com/user/Ttime5'>Reddit</a>, if you think that this was a mistake."
            )
    except:
        pass
    ...
    xp = int(
        float(league_days_learned)
        * (
            (6 * float(league_time) * 1)
            + (2 * int(league_reviews) * float(retention_bonus))
        )
    )
    ...
    if auth == 200:
        ...
        if Update_League == True:
            if c.execute(
                "SELECT username FROM League WHERE username = (?)", (User,)
            ).fetchone():
                c.execute(
    ...
    if auth == 404:
        c.execute(
            "INSERT INTO Leaderboard (Username, Streak, Cards , Time_Spend, Sync_Date, Month, Country, Retention, Token, version) VALUES(?, ?, ?, ?, ?, ?, ?, ?,?,?)",
            (
                User,
                Streak,
                Cards,
...
def delete(request):
    ...
    if auth == 200:
        group_list = c.execute(
            "SELECT groups FROM Leaderboard WHERE Username = (?)", (User,)
        ).fetchone()[0]
        if not group_list:
            groups = []
        else:
...
def create_group(request):
    ...
    if c.execute(
        ...
        c.execute(
            ...
            (Group_Name, 1, Pwd, json.dumps([User]), json.dumps([]), 0),
        )
        conn.commit()
...
def leaveGroup(request):
    ...
    if authUser == 200:
        ...
        c.execute(
            "UPDATE Leaderboard SET groups = (?) WHERE Username = (?) ",
            (json.dumps(group_list), user),
        )
        conn.commit()
        c.execute(
            "UPDATE Leaderboard SET Subject = (?) WHERE Username = (?) ", ("", user)
        )
        conn.commit()
        members = c.execute(
            "SELECT members FROM Groups WHERE Group_Name = (?)", (group,)
...
def getUserinfo(request):
    ...
    if a:
        country = c.execute(
            "SELECT Country FROM Leaderboard WHERE Username = (?)", (user,)
        ).fetchone()[0]
        group = c.execute(
            "SELECT Subject, groups FROM Leaderboard WHERE Username = (?)", (user,)
        ).fetchone()
        g_old = group[0]
        if not group[1]:
            g_new = []
        ...
        if g_old not in [group.replace(" ", "") for group in g_new]:
            g_new.append(g_old)
        league = c.execute(
            "SELECT league, history FROM League WHERE username = (?)", (user,)
        ).fetchone()
        status = c.execute(
            "SELECT Status FROM Leaderboard WHERE Username = (?)", (user,)
        ).fetchone()[0]
        return HttpResponse(json.dumps([country, g_new, league[0], league[1], status]))
    else:
        u1 = c.execute(
            "SELECT Country, Subject FROM Leaderboard WHERE Username = (?)", (user,)
        ).fetchone()
        u2 = c.execute(
            "SELECT league, history FROM League WHERE username = (?)", (user,)
        ).fetchone()
...
def season(request):
    return HttpResponse(
        json.dumps([[2022, 10, 12, 0, 0, 0], [2022, 10, 26, 0, 0, 0], "Season 53"])
    )
