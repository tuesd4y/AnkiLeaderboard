...
def rewrite_history(username, league, counter):
    data = c.execute(
        "SELECT xp, history FROM League WHERE username = (?)", (username,)
    ).fetchone()
    xp = data[0]
    try:
    ...
    c.execute(
        """UPDATE League SET history = (?) WHERE username = (?) """,
        (json.dumps(new_history), username),
    )


c.execute(
    "SELECT username, league, xp FROM League WHERE suspended IS NULL ORDER BY xp DESC"
)
