...
def reviews(request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT Username, Cards, Sync_Date FROM Leaderboard WHERE suspended IS NULL ORDER BY Cards DESC"
    )
...
def time(request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT Username, Time_Spend, Sync_Date FROM Leaderboard WHERE suspended IS NULL ORDER BY Time_Spend DESC"
    )
...
def streak(request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT Username, Streak, Sync_Date FROM Leaderboard WHERE suspended IS NULL ORDER BY Streak DESC"
    )
...
def retention(request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT Username, Retention, Sync_Date FROM Leaderboard WHERE suspended IS NULL ORDER BY Retention DESC"
    )
...
def alpha(request):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT username, xp, time_spend, reviews, retention, league, days_learned FROM League WHERE suspended IS NULL ORDER BY xp DESC"
    )
