...
class start_config(QDialog):
    ...
    def setValues(self):
        ...
        icon = QIcon()
        icon.addPixmap(
            QPixmap(f"{root}/designer/icons/settings.png"),
            QIcon.Mode.Normal,
            QIcon.State.Off,
        )
        ...
        self.dialog.Default_Tab.setToolTip(
            "This affects the Leaderboard and, if enabled, the home screen leaderboard."
        )
        self.dialog.sortby.setToolTip(
            "This affects the Leaderboard and, if enabled, the home screen leaderboard."
        )
        self.dialog.newday.setToolTip(
            "This needs to be the same as in Ankis' preferences."
        )
        self.dialog.autosync.setToolTip(
            "It will take a few extra seconds before you return to the homescreen after answering the last due card in a deck."
        )
        ...
        for i in country_list:
            self.dialog.country.setItemText(
                country_list.index(i), _translate("Dialog", i)
            )
    ...
    def checkLineEdit(self):
        ...
        if index == 0 or index == 3:
            if email and username and pwd and pwd_repeat:
                if pwd == pwd_repeat:
                    self.dialog.account_button.setEnabled(True)
                    self.dialog.account_pwd_repeat.setStyleSheet(
    ...
    def deleteAccount(self):
        ...
        pwd = self.dialog.account_pwd.text()
        if askUser(
            "<h3>Deleting Account</h3>If you delete your account, all your data will be deleted. <br><br><b>Do you want to delete your account now?</b>"
        ):
            data = {"username": username, "pwd": pwd}
    ...
    def changeUsername(self):
        ...
        if askUser(
            "If someone added you as a friend, they will have to re-add you after you changed your username.<br><br><b>Do you want to proceed?</b>"
        ):
    ...
    def status(self):
        ...
        if len(statusMsg) > 280:
            showWarning(
                "The message can only be 280 characters long.", title="Leaderboard"
            )
            return
        data = {
            "status": statusMsg,
