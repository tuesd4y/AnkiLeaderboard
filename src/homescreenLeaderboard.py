...
class homescreenLeaderboard:
        ...
        if config["tab"] != 4:
            ...
            if timeNow < newDay:
                startDay = datetime.datetime.combine(
                    date.today() - timedelta(days=1), newDay
                )
            else:
            ...
            for i in self.data[0]:
                ...
                if syncDate > startDay and username not in config["hidden_users"]:
                    if config["tab"] == 0:
                        counter += 1
                        self.lbList.append(
                            [counter, username, cards, time, streak, month, retention]
                        )
        ...
        if config["tab"] == 4:
            ...
            for i in self.data[1]:
                ...
                if leagueName == userLeagueName and xp != 0:
                    counter += 1
                    self.lbList.append(
                        [counter, username, xp, reviews, time, retention, daysLearned]
                    )
    ...
    def on_deck_browser_will_render_content(self, overview, content):
        ...
        if config["focus_on_user"] and len(self.lbList) > config["maxUsers"]:
    ...
    def deleteLeaderboard(self):
        gui_hooks.deck_browser_will_render_content.remove(
            self.on_deck_browser_will_render_content
        )
        DB = DeckBrowser(mw)
        DB.refresh()
...
DeckBrowser._linkHandler = wrap(
    DeckBrowser._linkHandler, deckbrowser_linkHandler_wrapper, "after"
)
