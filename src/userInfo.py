...
class start_user_info(QDialog):
    ...
    def setupUI(self):
        ...
        if response:
            ...
            if response[3]:
                ...
                if history["gold"] > 0:
                    medals = (
                        f"{medals} {history['gold'] if history['gold'] != 1 else ''}🥇"
                    )
                if history["silver"] > 0:
                ...
                for i in results["leagues"]:
                    ...
                    self.dialog.history.setItem(
                        rowPosition, 3, QtWidgets.QTableWidgetItem(str(i))
                    )

                    item = QtWidgets.QTableWidgetItem()
                    item.setData(
                        QtCore.Qt.ItemDataRole.DisplayRole,
                        int(results["seasons"][index]),
                    )
                    ...
                    item = QtWidgets.QTableWidgetItem()
                    item.setData(
                        QtCore.Qt.ItemDataRole.DisplayRole, int(results["xp"][index])
                    )
                    ...
                    item = QtWidgets.QTableWidgetItem()
                    item.setData(
                        QtCore.Qt.ItemDataRole.DisplayRole, int(results["rank"][index])
                    )
    ...
    def hideUser(self):
        ...
        tooltip(
            f"{self.user_clicked} will be hidden next time you open the leaderboard."
        )
