...
def Stats(season_start, season_end):
    ...
    league_reviews, league_retention = league_reviews_and_retention(
...
def streak(config, new_day, time_now):
    ...
    date_list = mw.col.db.list(
        "SELECT DISTINCT strftime('%Y-%m-%d', datetime((id - ?) / 1000, 'unixepoch', 'localtime')) FROM revlog WHERE ease > 0 ORDER BY id DESC;",
        new_day_shift_in_ms,
    )
...
def league_days_learned(season_start, season_end, new_day, time_now):
    date_list = [
        datetime.datetime.combine(season_start, new_day) + timedelta(days=x)
        for x in range((season_end - season_start).days + 1)
    ]
    days_learned = 0
    days_over = 0
    for i in date_list:
