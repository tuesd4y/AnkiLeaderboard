...
class start_main(QDialog):
    ...
    def setupUI(self):
        ...
        icon = QIcon()
        icon.addPixmap(
            QPixmap(f"{root}/designer/icons/krone.png"),
            QIcon.Mode.Normal,
            QIcon.State.Off,
        )
        ...
        header1.sectionClicked.connect(
            lambda: self.updateTable(self.dialog.Global_Leaderboard)
        )
        header2 = self.dialog.Friends_Leaderboard.horizontalHeader()
        header2.sectionClicked.connect(
            lambda: self.updateTable(self.dialog.Friends_Leaderboard)
        )
        header3 = self.dialog.Country_Leaderboard.horizontalHeader()
        header3.sectionClicked.connect(
            lambda: self.updateTable(self.dialog.Country_Leaderboard)
        )
        header4 = self.dialog.Custom_Leaderboard.horizontalHeader()
        header4.sectionClicked.connect(
            lambda: self.updateTable(self.dialog.Custom_Leaderboard)
        )
        ...
        for i in range(0, len(self.config["groups"])):
            ...
            self.dialog.groups.setItemText(
                i, _translate("Dialog", self.config["groups"][i])
            )
        self.dialog.groups.setCurrentText(self.config["current_group"])
        self.dialog.groups.currentTextChanged.connect(
            lambda: self.updateTable(self.dialog.Custom_Leaderboard)
        )
        self.dialog.Parent.setCurrentIndex(self.config["tab"])

        self.dialog.Global_Leaderboard.doubleClicked.connect(
            lambda: self.user_info(self.dialog.Global_Leaderboard)
        )
        self.dialog.Global_Leaderboard.setToolTip("Double click on user for more info.")
        self.dialog.Friends_Leaderboard.doubleClicked.connect(
            lambda: self.user_info(self.dialog.Friends_Leaderboard)
        )
        self.dialog.Friends_Leaderboard.setToolTip(
            "Double click on user for more info."
        )
        self.dialog.Country_Leaderboard.doubleClicked.connect(
            lambda: self.user_info(self.dialog.Country_Leaderboard)
        )
        self.dialog.Country_Leaderboard.setToolTip(
            "Double click on user for more info."
        )
        self.dialog.Custom_Leaderboard.doubleClicked.connect(
            lambda: self.user_info(self.dialog.Custom_Leaderboard)
        )
        self.dialog.Custom_Leaderboard.setToolTip("Double click on user for more info.")
        self.dialog.League.doubleClicked.connect(
            lambda: self.user_info(self.dialog.League)
        )
        self.dialog.League.setToolTip("Double click on user for more info.")
        self.dialog.league_label.setToolTip(
            "Leagues (from lowest to highest): Delta, Gamma, Beta, Alpha"
        )
    ...
    def switchGroup(self):
        ...
        for i in self.groups_lb:
            if self.dialog.groups.currentText().replace(" ", "") in i[6]:
                self.add_row(
                    self.dialog.Custom_Leaderboard, i[0], i[1], i[2], i[3], i[4], i[5]
                )
    ...
    def buildLeaderboard(self):
        ...
        if time_now < new_day:
            start_day = datetime.datetime.combine(
                date.today() - timedelta(days=1), new_day
            )
        else:
        ...
        for i in self.response[0]:
            ...
            if (
                sync_date > start_day
                and username.split(" |")[0] not in self.config["hidden_users"]
            ):
                self.add_row(
                    self.dialog.Global_Leaderboard,
                    username,
                ...
                if any(i in c_groups for i in groups):
                    self.groups_lb.append(
                        [username, cards, time, streak, month, retention, groups]
                    )
    ...
    def highlight(self, tab):
        for i in range(tab.rowCount()):
            ...
            if (
                item in self.config["friends"]
                and tab != self.dialog.Friends_Leaderboard
            ):
                for j in range(tab.columnCount()):
                    tab.item(i, j).setBackground(
